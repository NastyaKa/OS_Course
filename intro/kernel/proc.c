...
void dump(void) {
  uint64 *p = (uint64 *)&(myproc()->trapframe->s2);
  uint i;
  for (i = 0; i < 10; i++) {
    printf("%d\n", (int)p[i]);
  }
...
int copyouter(struct proc *cur_p, struct proc *p, int register_num,
              uint64 *return_value) {
  if (copyout(cur_p->pagetable, *return_value,
              (char *)(&p->trapframe->s2 + register_num - 2), 4) == -1) {
...
int dump2(int pid, int register_num, uint64 *return_value) {
  ...
  if (register_num < 2 || register_num > 11) {
    ...
    return -3;
  } else if (cur_p->pid == pid) {
    return copyouter(cur_p, cur_p, register_num, return_value);
  }

  for (p = proc; p < &proc[NPROC]; p++) {
    if (p->pid == pid) {
      if (cur_p->pid == p->parent->pid) {
        return copyouter(cur_p, p, register_num, return_value);
      } else {
