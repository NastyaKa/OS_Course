...
void bit_xor(Sz_info *array, int index, int k) {
  if (k < MAXSIZE) {
    index >>= 1;
    if (bit_isset(array[k].alloc, index)) {
      bit_clear(array[k].alloc, index);
    } else {
      bit_set(array[k].alloc, index);
    }
  }
}
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_xor(bd_sizes, blk_index(k, p), k);
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_xor(bd_sizes, blk_index(k - 1, p), k - 1);
...
int bd_initfree_pair(int k, int bi, int bir) {
  int free = 0;
  ...
  if (bi < bir && bit_isset(bd_sizes[k].alloc, bir >> 1)) {
    free += BLK_SIZE(k);
    int buddy = (bir % 2 == 0) ? bir + 1 : bir - 1;
...
int bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {  // skip max size
    ...
    free += bd_initfree_pair(k, left, right);
  }
  return free;
}
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {
    ...
    sz = sizeof(char) * ROUNDUP(NBLK(k), 16) >> 4;
    bd_sizes[k].alloc = p;
    memset(bd_sizes[k].alloc, 0, sz);
    p += sz;
  }
  if (MAXSIZE > 0) {
    lst_init(&bd_sizes[MAXSIZE].free);
  }

  // allocate the split array for each size k, except for k = 0, since
  // we will not split blocks of size k = 0, the smallest size.
  for (int k = 1; k < nsizes; k++) {
    sz = sizeof(char) * (ROUNDUP(NBLK(k), 8)) >> 3;
    bd_sizes[k].split = p;
